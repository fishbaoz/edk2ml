  ...
  )
{
  ...
  NBPtr->FamilySpecificHook[DllPDBypassFenceThresholdTxPadTraining] = MemFDllPDBypassFenceThresholdTxPadTraining;
  NBPtr->FamilySpecificHook[DllPDBypassWrDatTraining] = MemFDllPDBypassWrDatTraining;
  NBPtr->FamilySpecificHook[DllPDBypassRepeatRxEnTrn] = MemFDllPDBypassRepeatRxEnTrn;
...
/**
 *
 *     This function bypasses FenceThresholdTxPad training by fixing Threshold at 10h
 *
 *     @param[in,out] *NBPtr    - Pointer to the MEM_NB_BLOCK
 *     @param[in,out] *OptParam - Unused
 *
 *     @return  TRUE - FenceThresholdTxPad training not bypassed
 *     @return  FALSE - FenceThresholdTxPad training bypassed
*/
BOOLEAN
MemFDllPDBypassFenceThresholdTxPadTraining (
  IN OUT   MEM_NB_BLOCK *NBPtr,
  IN OUT   VOID* OptParam
  )
{
  if (NBPtr->IsSupported[SelectMotherboardPower] && NBPtr->RefPtr->EnableDllPDBypassMode) {
    IDS_HDT_CONSOLE (MEM_FLOW, "\nSkip FenceThresholdTxPad training in DllPDBypassMode.\n");
    MAKE_TSEFO (NBPtr->NBRegTable, DCT_PHY_ACCESS, 0x0C, 20, 16, BFPhyFence);
    MemNSetBitFieldNb (NBPtr, BFPhyFence, 0x10);
    // BIOS rewrites F2x[1, 0]9C_x04, DRAM Address/Command Timing Control
...
/**
 *
 *     This function bypasses WrDat training by fixing WrDatDly at {0, 10h}
 *
 *     @param[in,out] *NBPtr    - Pointer to the MEM_NB_BLOCK
 *     @param[in,out] *OptParam - Unused
 *
 *     @return  TRUE - WrDat training not bypassed
 *     @return  FALSE - WrDat training bypassed
*/
BOOLEAN
MemFDllPDBypassWrDatTraining (
  IN OUT   MEM_NB_BLOCK *NBPtr,
  IN OUT   VOID* OptParam
  )
{
  ...
  if (NBPtr->IsSupported[SelectMotherboardPower] && NBPtr->RefPtr->EnableDllPDBypassMode) {
    ...
    // Program D18F2x9C_x0000_0[3:0]0[3:1]_dct[0]_mp[1:0][WrDatGrossDly, WrDatFineDly] = {0, 10h)
    IDS_HDT_CONSOLE (MEM_FLOW, "\nSkip WrDat training in DllPDBypassMode.\n");
    for (ByteLane = 0; ByteLane < (NBPtr->MCTPtr->Status[SbEccDimms] ? 9 : 8); ByteLane++) {
      // This includes ECC as byte 8
      NBPtr->ChannelPtr->WrDatDlys[(NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay) * NBPtr->TechPtr->DlyTableWidth () + ByteLane] = 0x10;
      NBPtr->SetTrainDly (NBPtr, AccessWrDatDly, DIMM_BYTE_ACCESS (NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay, ByteLane), 0x10);
    }
    // return FALSE to skip regular WrDat training
    return FALSE;
  } else {
    return TRUE;
  }
      ...
      if ((DCTPtr->Timings.CsEnabled & ((UINT16) ((NBPtr->CsPerDelay == 2)? 3 : 1) << ChipSel)) != 0) {
        if (!(NBPtr->MCTPtr->Status[SbLrdimms]) || ((NBPtr->ChannelPtr->LrDimmPresent & ((UINT8) 1 << (ChipSel >> 1))) != 0)) {
          ...
          for (TechPtr->TrnNibble = NIBBLE_0; TechPtr->TrnNibble <= (NBPtr->FamilySpecificHook[TrainWlPerNibble] (NBPtr, &Dimm)? NIBBLE_0 : NIBBLE_1); TechPtr->TrnNibble++) {
            ...
            for (ByteLane = 0; ByteLane < (NBPtr->MCTPtr->Status[SbEccDimms] ? 9 : 8); ByteLane++) {
              // This includes ECC as byte 8
              NBPtr->ChannelPtr->WrDqsDlys[Dimm * TechPtr->DlyTableWidth () + ByteLane] = 0;
...
/**
 *
 *     This function repeats Rx En training @ 667
 *
 *     @param[in,out] *NBPtr    - Pointer to the MEM_NB_BLOCK
 *     @param[in,out] *OptParam - Unused
 *
 *     @return  TRUE - Rx En training repeated
 *     @return  FALSE - Rx En training not repeated
*/
BOOLEAN
MemFDllPDBypassRepeatRxEnTrn (
  IN OUT   MEM_NB_BLOCK *NBPtr,
  IN OUT   VOID* OptParam
  )
{
  if (NBPtr->IsSupported[SelectMotherboardPower] && NBPtr->RefPtr->EnableDllPDBypassMode) {
    if (NBPtr->DCTPtr->Timings.TargetSpeed == DDR667_FREQUENCY) {
      NBPtr->FamilySpecificHook[DllPDBypassTxRxEn] (NBPtr, NULL);
      memTrainSequenceDDR3[NBPtr->TrainingSequenceIndex].MemTechFeatBlock->HwBasedDQSReceiverEnableTrainingPart1 (NBPtr->TechPtr);
      return TRUE;
    }
  }
  return FALSE;
  ...
  )
{
  ...
  if (NBPtr->IsSupported[SelectMotherboardPower] && NBPtr->RefPtr->EnableDllPDBypassMode) {
    ...
    for (ByteLane = 0; ByteLane < MAX_BYTELANES_PER_CHANNEL; ByteLane++) {
      // This includes ECC as byte 8
      MemNSetTrainDlyNb (NBPtr, AccessPhRecDly, DIMM_BYTE_ACCESS (NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay, ByteLane), 0x10);
      IDS_HDT_CONSOLE (MEM_FLOW, "%02x ", 0x10);
    }
    IDS_HDT_CONSOLE (MEM_FLOW, "\n\t\t\t   PhyFenceTrEn = 1");
    ...
    IDS_HDT_CONSOLE (MEM_FLOW, "\n\nReprogram fence and WrDqs, RxEn delays\n");
    // Re-run [2.10.8.4.3 Phy Fence Programming], skip training all fence training outcomes as 0x10 instead.
    MAKE_TSEFO (NBPtr->NBRegTable, DCT_PHY_ACCESS, 0x0C, 30, 16, BFPhyFence);
    MemNSetBitFieldNb (NBPtr, BFPhyFence, 0x4210);
    Fence2Data = 0;
    MemNSetBitFieldNb (NBPtr, BFDataFence2, Fence2Data);
    NBPtr->FamilySpecificHook[ProgramFence2RxDll] (NBPtr, &Fence2Data);

    // Reprogram WrDqs and RxEn delays.
    for (ByteLane = 0; ByteLane < MAX_BYTELANES_PER_CHANNEL; ByteLane++) {
      WrDqsDly = NBPtr->ChannelPtr->WrDqsDlys[(NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay) * NBPtr->TechPtr->DlyTableWidth () + ByteLane];
      MemNSetTrainDlyNb (NBPtr, AccessWrDqsDly, DIMM_BYTE_ACCESS ((NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay), ByteLane), WrDqsDly);
      RcvEnDly = NBPtr->ChannelPtr->RcvEnDlys[(NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay) * NBPtr->TechPtr->DlyTableWidth () + ByteLane];
      MemNSetTrainDlyNb (NBPtr, AccessRcvEnDly, DIMM_BYTE_ACCESS ((NBPtr->TechPtr->ChipSel / NBPtr->CsPerDelay), ByteLane), RcvEnDly);
    }

    // Reprogram D18F2x9C_x0000_0004_dct[0]_mp[1:0].
    MemNSetBitFieldNb (NBPtr, BFAddrTmgControl, MemNGetBitFieldNb (NBPtr, BFAddrTmgControl));
