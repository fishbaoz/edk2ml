 ...
 * mndctml.c
 *
 * Northbridge DCT support for ML
 *
 * @xrefitem bom "File Content Label" "Release Content"
 * @e project: AGESA
 * @e sub-project: (Mem/NB/ML)
 * @e \$Revision: 282281 $ @e \$Date: 2014-01-09 03:26:33 -0600 (Thu, 09 Jan 2014) $
 *
 **/
/*****************************************************************************
*
...
#define FILECODE PROC_MEM_NB_ML_MNDCTML_FILECODE
  ...
  )
{
  ...
  MemNSetBitFieldNb (NBPtr, BFWrOdtOnDuration, DEFAULT_WR_ODT_ML);
  MemNSetBitFieldNb (NBPtr, BFRdOdtOnDuration, DEFAULT_RD_ODT_ML);
  MemNSetBitFieldNb (NBPtr, BFWrOdtTrnOnDly, DEFAULT_RD_ODT_TRNONDLY_ML);
  ...
  )
{
  ...
  P = MemNTotalSyncComponentsML (NBPtr);
  ...
  *MaxDlyPtr = 0x3FF;
  ...
  // IF (NclkFreq/MemClkFreq < 2) THEN 0.5 MEMCLK ELSE 0 ENDIF)
  N = 1;
  P = 3 + (((NBPtr->NBClkFreq / ((NBPtr->MemPstate == MEMORY_PSTATE0) ? NBPtr->DCTPtr->Timings.Speed : DDR667_FREQUENCY)) < 2) ? 1 : 0);
  ...
  )
{
  ...
  for (Dct = 0; Dct < MAX_DCTS_PER_NODE_ML; Dct++) {
    ...
    if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
      ...
      IEM_INSERT_CODE (IEM_BEFORE_DRAM_INIT, IemBeforeDramInitOverrideML, (NBPtr));
    }
  }
}
...
/**
 *
 *      This function modifies CS interleaving low address according to several conditions for ML.
 *
 *     @param[in,out]  *NBPtr    - Pointer to the MEM_NB_BLOCK
  ...
  )
{
  ...
  if (NBPtr->MemPstateStage == MEMORY_PSTATE_1ST_STAGE) {
    ...
    for (Dct = 0; Dct < MAX_DCTS_PER_NODE_ML; Dct++) {
      ...
      if (NBPtr->DCTPtr->Timings.DctMemSize != 0) {
        ...
        for (AccessType = AccessRcvEnDly; AccessType <= AccessWrDqsDly; AccessType ++) {
          for (ChipSel = 0; ChipSel < MAX_CS_PER_CHANNEL_ML; ChipSel = ChipSel + NBPtr->CsPerDelay) {
    ...
    NBPtr->FamilySpecificHook[DllPDBypassRepeatRxEnTrn] (NBPtr, NULL);
