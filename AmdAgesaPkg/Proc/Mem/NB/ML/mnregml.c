 ...
 * mnregml.c
 *
 * Common Northbridge register related functions for ML
 *
...
#define FILECODE PROC_MEM_NB_ML_MNREGML_FILECODE
...
STATIC CONST UINT8 InstancesPerTypeML[8] = {9, 3, 1, 2, 2, 0, 1, 1};
 ...
 *
 *     @return          TRUE -  This node is a ML.
 *     @return          FALSE - This node is not a ML.
 *
 */
BOOLEAN
MemNIsIdSupportedML (
...
{
  if (((LogicalIdPtr->Family & AMD_FAMILY_16_ML) != 0)
  ...
  )
{
  ...
  if (FieldName == BFDctAccessDone) {
    // No need to poll DctAccessDone for ML due to enhancement in phy
    Value = 1;
    ...
    if (Address) {
      ...
      if ((Type == DCT_PHY_ACCESS) && IsPhyDirectAccess) {
        ...
        if (IsWholeRegAccess) {
          ...
          if ((Address & 0x0F00) == 0x0F00) {
            ...
            NumOfInstances = InstancesPerTypeML[(Address >> 13) & 0x7];
            if (!IsSet) {
      ...
      for (Instance = 0; Instance < NumOfInstances; Instance++) {
        if (Type == NB_ACCESS) {
          ...
          // DCT_EXTRA_ACCESS is not supported on ML
          ASSERT (FALSE);
        }
        ...
        if (IsSet) {
          ...
          do {
            ...
            if (IsLinked) {
              MemNCmnGetSetFieldML (NBPtr, 1, FieldName + 1, Field >> (Highbit - Lowbit + 1));
            }
          ...
          if (IsLinked) {
            Value |= MemNCmnGetSetFieldML (NBPtr, 0, FieldName + 1, 0) << (Highbit - Lowbit + 1);
          }
  ...
  )
{
  ...
  if (!MemNAllocateNBRegTableNb (NBPtr, NbRegTabML)) {
  ...
  MAKE_TSEFO (NBRegTable, DCT_PHY_DIRECT, 0x0D0F0F1E,  7,  7, BFDllPDBypass);

  MAKE_TSEFO (NBRegTable, DCT_PHY_DIRECT, 0x0D0F0F1E, 14, 12, BFDllCSRBiasTrim);
  MAKE_TSEFO (NBRegTable, DCT_PHY_DIRECT, 0x0D0F2F1E,  7,  7, BFClkDllPDBypass);
  MAKE_TSEFO (NBRegTable, DCT_PHY_DIRECT, 0x0D0F8F1E,  7,  7, BFCmdDllPDBypass);
  MAKE_TSEFO (NBRegTable, DCT_PHY_DIRECT, 0x0D0FC01E,  7,  7, BFAddrDllPDBypass);
  ...
  MAKE_TSEFO (NBRegTable, DCT_PHY_DIRECT, 0x0D0F0F1F, 14, 14, BFLowPowerRcvrEn);
  ...
  MAKE_TSEFO (NBRegTable, NB_ACCESS, _FN (3, 0xE8), 3, 3, BFEccDis);
  ...
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFDllPDBypass);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFDllCSRBiasTrim);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFClkDllPDBypass);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFCmdDllPDBypass);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFAddrDllPDBypass);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFDataRxVioLvl);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFRxDqInsDly);
  MULTI_MPSTATE_COPY_TSEFO (NBRegTable, BFLowPowerRcvrEn);
