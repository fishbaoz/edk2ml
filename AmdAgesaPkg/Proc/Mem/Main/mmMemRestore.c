 ...
 * @e \$Revision: 282917 $ @e \$Date: 2014-01-20 02:33:10 -0600 (Mon, 20 Jan 2014) $
 *
 **/
/*****************************************************************************
*
  ...
  if (RefPtr->SaveMemContextCtl) {
    ...
    if (!RefPtr->MemRestoreCtl) {
      ...
      if (S3NBPtr != NULL) {
        ...
        BufferSize = 0;
        LibAmdMemFill (DeviceDescript, 0x00, MAX_NODES_SUPPORTED * sizeof (DESCRIPTOR_GROUP), StdHeader);
        for (Node = 0; Node < MemMainPtr->DieCount; Node ++) {
    ...
    if (RefPtr->MemContext.NvStorage != NULL) {
      ...
      if (S3NBPtr != NULL) {
        ...
        if ((RefPtr->IsCapsuleMode && (AmdMemS3Resume (&(MemPtr->StdHeader)) == AGESA_SUCCESS)) ||
          ...
          for (Node = 0; Node < MemMainPtr->DieCount; Node ++) {
            if (IsFeatureEnabled (C6Cstate, MemMainPtr->MemPtr->PlatFormConfig, &(MemMainPtr->MemPtr->StdHeader)) || NBArray[Node].MemS3PspDetect (&NBArray[Node])) {
