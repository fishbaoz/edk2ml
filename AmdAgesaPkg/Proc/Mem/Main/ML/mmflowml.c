 ...
 * mmflowml.c
 *
 * Main Memory initialization sequence for ML
 *
...
#define FILECODE PROC_MEM_MAIN_ML_MMFLOWML_FILECODE
/* features */
#include "mftds.h"
 ...
 *      systems that only support ML processors.
 *
  ...
  )
{
  ...
  //----------------------------------------------------------------
  MemNConfigureDctNormalML (&NBPtr[BSP_DIE]);
  ...
  IEM_INSERT_CODE (IEM_LATE_DCT_CONFIG, IemLateDctConfigML, (NBPtr));
  ...
  // Power Saving
  //----------------------------------------------------------------
  if (!MemNPowerSavingML (&NBPtr[BSP_DIE])) {
    return AGESA_FATAL;
  }
