...
#include "Filecode.h"
#define FILECODE PROC_FCH_AVALON_AVALONSPI_AVALONLPCENVSERVICE_FILECODE

/**
 * FchInitAvalonEnvLpcPciTable - PCI device registers initial
 * during early POST.
 *
 */
REG8_MASK FchInitAvalonEnvLpcPciTable[] =
{
  //
  ...
  {FCH_LPC_REGBB, 0xF2, BIT0 + BIT2 + BIT3},                /// Enabled SPI Prefetch from HOST.
  {0xFF, 0xFF, 0xFF},
};
  ...
  )
{
  ...
  ProgramPciByteTable ((REG8_MASK*) (&FchInitAvalonEnvLpcPciTable[0]), sizeof (FchInitAvalonEnvLpcPciTable) / sizeof (REG8_MASK), StdHeader);
