 ...
 * @e project:     AGESA
 * @e sub-project: FCH
 * @e \$Revision: 285493 $   @e \$Date: 2014-02-21 12:53:25 -0600 (Fri, 21 Feb 2014) $
 *
 */
/*;********************************************************************************
;
...
#include "Filecode.h"
#define FILECODE PROC_FCH_AVALON_AVALONUSB_AVALONXHCIENVSERVICE_FILECODE
  ...
  )
{
  ...
  if (CheckAvalonRevisionA0 (StdHeader)) {
    ...
    RwXhci0IndReg ( FCH_XHCI_IND60_REG08, 0x80FC00FF, 0x00020300 , StdHeader);
  }
  ...
  )
{
  ...
  RwMem (ACPI_MMIO_BASE + XHCI_BASE + XHCI_ACPI_MMIO_AMD_REG30, AccessWidth32, ~(UINT32) (BIT0 + BIT4 + BIT24), BIT24);
  // DcpNumP1En
  RwMem (ACPI_MMIO_BASE + XHCI_BASE + XHCI_ACPI_MMIO_AMD_REG48, AccessWidth32, 0x00000000, FCH_XHCI_IND_REG100);
  RwMem (ACPI_MMIO_BASE + XHCI_BASE + XHCI_ACPI_MMIO_AMD_REG4C, AccessWidth32, ~(UINT32)  BIT5, BIT5);
  ...
  //A1 bit RwMem (ACPI_MMIO_BASE + XHCI_BASE + XHCI_ACPI_MMIO_AMD_REG28, AccessWidth32, ~(UINT32) (BIT20), (BIT20));
  ...
  RwMem (ACPI_MMIO_BASE + XHCI_BASE + XHCI_ACPI_MMIO_AMD_REG30, AccessWidth32, ~(UINT32) (BIT27), 0);
