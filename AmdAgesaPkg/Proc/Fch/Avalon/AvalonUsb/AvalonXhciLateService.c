 ...
 * @file
 *
 * Config AVALON FCH USB3 controller
 *
 * Init USB3 features.
 *
...
#include "Filecode.h"
#define FILECODE PROC_FCH_AVALON_AVALONUSB_AVALONXHCILATESERVICE_FILECODE

//
// Declaration of local functions
//
VOID FchUsbLsDeviceWA     (IN VOID *FchDataPtr);
VOID FchUsbLsDeviceWAXhci (IN VOID *FchDataPtr);
  ...
  )
{
  ...
  // xHCI controller PCI configuration space "Read Only" registers write lock enable
  RwXhci0IndReg ( FCH_XHCI_IND_REG04, ~(UINT32) BIT8, BIT8, StdHeader);
  if (( LocalCfgPtr->Misc.S3Resume == 1 ) && (CheckAvalonRevisionA0 (StdHeader))) {
    FchUsbLsDeviceWA (FchDataPtr);
  }
}
/**
 * FchUsbLsDeviceWA - Workaround for USB MS S3 issue
 *
 *
 *
 * @param[in] FchDataPtr Fch configuration structure pointer.
  ...
  )
{
  ...
  FchUsbLsDeviceWAXhci (FchDataPtr);
}
/**
 * FchUsbLsDeviceWAXhci - Workaround for USB MS S3 issue on Xhci ports
 *
 *
 *
 * @param[in] FchDataPtr Fch configuration structure pointer.
  ...
  )
{
  ...
  LocalCfgPtr = (FCH_DATA_BLOCK *)FchDataPtr;
  StdHeader = LocalCfgPtr->StdHeader;
  PortSC_Mask = FCH_XHCI_PORTSC_CCS + \
                FCH_XHCI_PORTSC_PED + \
                FCH_XHCI_PORTSC_PLS + \
                FCH_XHCI_PORTSC_SPEED;
  Check_Value = BIT0 + BIT1 + 0x800 + 0x1E0;

  ReadPci ((USB_XHCI_BUS_DEV_FUN << 16) + 0x10, AccessWidth32, &Bar0, StdHeader);
  if ((Bar0 != 0) && (Bar0 != 0xFFFFFFFF)) {
    Bar0 &= 0xFFFFFFF0;
    for (Port = 0; Port < 4; Port++) {
      PortSC = ACPIMMIO32 (Bar0 + FCH_XHCI_BAR_REG420 + 0x10 * Port);
      if ((PortSC & PortSC_Mask) == Check_Value) {
        PortSC |= FCH_XHCI_PORTSC_PED;
        ACPIMMIO32 (Bar0 + FCH_XHCI_BAR_REG420 + 0x10 * Port) = PortSC;
      }
    }
  }
}
