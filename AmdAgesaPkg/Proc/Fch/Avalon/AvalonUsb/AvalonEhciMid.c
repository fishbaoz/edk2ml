...
#include "Filecode.h"
#include "AvalonFch.h"
#define FILECODE PROC_FCH_AVALON_AVALONUSB_AVALONEHCIMID_FILECODE
  ...
  )
{
  ...
  DeviceId = (AVALON_EHCI1_BUS_DEV_FUN << 16);
  ...
  )
{
  ...
  DeviceId = (AVALON_EHCI2_BUS_DEV_FUN << 16);
  EhciInitAfterPciInit (DeviceId, FchDataPtr);
}
  ...
  )
{
  ...
  DeviceId = (AVALON_EHCI3_BUS_DEV_FUN << 16);
  EhciInitAfterPciInit (DeviceId, FchDataPtr);
}
  ...
  )
{
  ...
  DebugPortSel = LocalCfgPtr->Usb.Ehci2DebugPortSel;
  DeviceId = (AVALON_EHCI2_BUS_DEV_FUN << 16);
  EhciSetDebugPort (DeviceId, LocalCfgPtr, DebugPortSel);

  DebugPortSel = LocalCfgPtr->Usb.Ehci3DebugPortSel;
  DeviceId = (AVALON_EHCI3_BUS_DEV_FUN << 16);
  EhciSetDebugPort (DeviceId, LocalCfgPtr, DebugPortSel);
