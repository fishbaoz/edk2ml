...
#include  "Filecode.h"
#define FILECODE PROC_GNB_MODULES_GNBINITML_GFXLIBML_FILECODE
/*----------------------------------------------------------------------------------------
 ...
 */
UINT32
GfxLibGetSystemPllCofML (
  IN       AMD_CONFIG_PARAMS      *StdHeader
  )
{
  D0F0xBC_xC0500000_STRUCT  D0F0xBC_xC0500000;
  GnbRegisterReadML (GnbGetHandle (StdHeader), D0F0xBC_xC0500000_TYPE,  D0F0xBC_xC0500000_ADDRESS, &D0F0xBC_xC0500000.Value, 0, StdHeader);
  ...
  )
{
  UINT32  Divider;
  UINT32  SystemPllCof;
  SystemPllCof = GfxLibGetSystemPllCofML (StdHeader) * 100;
  if (Did >= 8 && Did <= 0x3F) {
    Divider = Did * 25;
  ...
  )
{
  ...
  GnbRegisterReadML (GnbGetHandle (GnbLibGetHeader (Gfx)), GMMx1740_TYPE, GMMx1740_ADDRESS, &GMMx1740.Value, 0, GnbLibGetHeader (Gfx));
