 ...
 * mrndctml.c
 *
 * Northbridge DCT support for ML Recovery
 *
...
#define FILECODE PROC_RECOVERY_MEM_NB_ML_MRNDCTML_FILECODE
...
VOID
MemRecNProgramNbPstateDependentRegistersML (
  IN OUT   MEM_NB_BLOCK *NBPtr
  )
{
 ...
 *
 *
 *  This function initializes the DRAM devices for ML
 *
 *     @param[in,out]   *NBPtr   - Pointer to the MEM_NB_BLOCK
 *
 */
  ...
  )
{
  ...
  // C. Program D18F2x9C_x0000_000B_dct[0] = 40000000h. CH0 only
  ...
  MemRecNProgramNbPstateDependentRegistersML (NBPtr);
  ...
  )
{
  ...
  P = MemRecNTotalSyncComponentsML (NBPtr);
