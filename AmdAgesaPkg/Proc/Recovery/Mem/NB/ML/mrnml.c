 ...
 * mrnml.c
 *
 * Common Northbridge  functions for ML Recovery
 *
 * @xrefitem bom "File Content Label" "Release Content"
 * @e project: AGESA
 * @e sub-project: (Proc/Recovery/Mem/NB/ML)
...
CODE_GROUP (G3_DXE)
RDATA_GROUP (G3_DXE)
#define FILECODE PROC_RECOVERY_MEM_NB_ML_MRNML_FILECODE
/*----------------------------------------------------------------------------
...
VOID
STATIC
MemRecNInitNBRegTableML (
  IN OUT   TSEFO *NBRegTable
  );

UINT32
STATIC
MemRecNCmnGetSetFieldML (
 ...
 *----------------------------------------------------------------------------
 */
STATIC CONST UINT32 RecModeDefRegArrayML[] = {
 ...
 *
 *     @return          TRUE  - This node is a ML and this NB block has been initialized
 *     @return          FALSE - This node is not a ML
 */

BOOLEAN
MemRecConstructNBBlockML (
  IN OUT   MEM_NB_BLOCK *NBPtr,
  ...
  )
{
  ...
  if (!MemRecNIsIdSupportedML (NBPtr, &(MemPtr->DiesPerSystem[NodeID].LogicalCpuid))) {
    return FALSE;
  }
  ...
  MemPtr->DiesPerSystem[NodeID + 1].DctCount = MAX_DCTS_PER_NODE_ML;
  ...
  MemRecNInitNBRegTableML (NBPtr->NBRegTable);
  NBPtr->Dct = 0;
  NBPtr->Channel = 0;
  ...
  NBPtr->InitRecovery = MemRecNMemInitML;

  NBPtr->RecModeDefRegArray = RecModeDefRegArrayML;

  NBPtr->SwitchNodeRec = (VOID (*) (MEM_NB_BLOCK *, UINT8)) MemRecDefRet;
  NBPtr->SwitchDCT = MemRecNSwitchDctML;
  NBPtr->SwitchChannel = MemRecNSwitchChannelML;
  NBPtr->SetMaxLatency = MemRecNSetMaxLatencyML;
  NBPtr->GetSysAddrRec = MemRecNGetMCTSysAddrNb;
  NBPtr->SendMrsCmd = MemRecNSendMrsCmdNb;
  NBPtr->sendZQCmd = MemRecNSendZQCmdNb;
  NBPtr->SetDramOdtRec = MemRecNSetDramOdtML;
  ...
  NBPtr->MemRecNCmnGetSetFieldNb = MemRecNCmnGetSetFieldML;
  NBPtr->MemRecNcmnGetSetTrainDlyNb = MemRecNcmnGetSetTrainDlyML;
  NBPtr->MemRecNSwitchDctNb = MemRecNSwitchDctML;
  NBPtr->TrainingFlow = MemNRecTrainingFlowUnb;
  NBPtr->MemRecNInitializeMctNb = MemRecNInitializeMctML;
  NBPtr->MemRecNFinalizeMctNb = MemRecNFinalizeMctML;
  ...
  )
{
  ...
  MemRecNSwitchChannelML (NBPtr, 0);
}
