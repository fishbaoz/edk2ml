  ...
  )
{
  ...
  PSTATE_CPU_FAMILY_SERVICES  *PstateFamilyServices;
  DOUBLE              P0Freq;
  UINT32              P0Frequency;
  DOUBLE              EnCnt;
  ...
  if ((EntryPoint & CPU_FEAT_BEFORE_PM_INIT) != 0) {
    // Get frequency of HWP0 before power delivery compatibility check
    PstateFamilyServices = NULL;
    GetFeatureServicesOfCurrentCore (&PstateFamilyServiceTable, (CONST VOID **)&PstateFamilyServices, StdHeader);
    ASSERT (PstateFamilyServices != NULL);

    PstateFamilyServices->GetPstateFrequency (PstateFamilyServices, 0, &P0Frequency, StdHeader);

    AllocateHeapParams.BufferHandle = AMD_SCS_HWP0_FREQ;
    AllocateHeapParams.Persist = HEAP_LOCAL_CACHE;
    AllocateHeapParams.RequestedBufferSize = sizeof (P0Frequency);
    ...
    *((UINT32 *) (AllocateHeapParams.BufferPtr)) = P0Frequency;
    ...
    P0Frequency = *((UINT32 *) LocateHeapParams.BufferPtr);

    // Convert from Mhz to GHz
    P0Freq = ((DOUBLE) (P0Frequency)) / 1000;

    //1. Calculate normalizer
    EnCnt = (DOUBLE) (254 - LocalMsrRegister.Base - HEADROOM);
    EnCnt = EnCnt / (P0Freq * EnCntConstant * SampleAndResidency.CSampleTimer / 2);
