 ...
 * @file
 *
 * AMD Family_16 Mullins Pstate feature support functions.
 *
...
#define FILECODE PROC_CPU_FAMILY_0X16_ML_F16MLPSTATE_FILECODE
...
VOID
STATIC
F16MlGetPowerStepValueInTime (
  IN OUT   UINT32  *PowerStepPtr
  );

VOID
STATIC
F16MlGetPllValueInTime (
  IN OUT   UINT32  *PllLockTimePtr
  );

AGESA_STATUS
STATIC
F16MlGetFrequencyXlatRegInfo (
...
BOOLEAN
STATIC
F16MlIsPstatePsdDependent (
  IN       PSTATE_CPU_FAMILY_SERVICES *PstateCpuServices,
  IN OUT   PLATFORM_CONFIGURATION *PlatformConfig,
  IN       AMD_CONFIG_PARAMS *StdHeader
  )
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlIsPstatePsdDependent\n");
  // Family 16h ML defaults to independent PSD
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPstateTransLatency\n");
  ...
  F16MlGetPowerStepValueInTime (&PowerStepDown);
  F16MlGetPowerStepValueInTime (&PowerStepUp);
  ...
  if (!CpuFidSameFlag) {
    ...
    F16MlGetPllValueInTime (&PllLockTime);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPstateFrequency - P%d\n", StateNumber);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPstatePower - P%d\n", StateNumber);
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPstateMaxState\n");

  LocalPciRegister = 0;

  // For F16 Mullins CPU, skip boosted p-state. The boosted p-state number = F4x15C[NumBoostStates].
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPstateRegisterInfo - P%d\n", PState);
...
CONST PSTATE_CPU_FAMILY_SERVICES ROMDATA F16MlPstateServices =
{
  0,
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPowerStepValueInTime\n");
...
{
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetPllValueInTime\n");
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlGetFrequencyXlatRegInfo - PstateNumber=%d, Frequency=%d\n", PStateNumber, Frequency);
