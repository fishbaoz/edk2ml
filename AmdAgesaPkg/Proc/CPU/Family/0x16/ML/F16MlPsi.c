 ...
 * @file
 *
 * AMD Family_16 Mullins PSI Initialization
 *
...
#define FILECODE PROC_CPU_FAMILY_0X16_ML_F16MLPSI_FILECODE
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlPmVrmLowPowerModeEnable\n");
  ...
  if (PlatformConfig->VrmProperties[NbVrm].LowPowerThreshold != 0) {
    ...
    for (NbPstate = 0; NbPstate <= NbPstateMaxVal; NbPstate++) {
      ...
      CurrentNbVid = (UINT32) GetF16MlNbVid (&NbPstateReg);
...
CONST PSI_FAMILY_SERVICES ROMDATA F16MlPsiSupport =
{
  0,
