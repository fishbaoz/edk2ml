 ...
 * @file
 *
 * AMD Family_16 Mullins after warm reset sequence for core P-states
 *
...
#define FILECODE PROC_CPU_FAMILY_0X16_ML_F16MLCOREAFTERRESET_FILECODE
...
VOID
STATIC
F16MlPmCoreAfterResetPhase1OnCore (
  IN       AMD_CONFIG_PARAMS *StdHeader
  );

VOID
STATIC
F16MlPmCoreAfterResetPhase2OnCore (
  IN       VOID              *HwPsMaxVal,
  IN       AMD_CONFIG_PARAMS *StdHeader
  );
...
/*---------------------------------------------------------------------------------------*/
/**
 * Support routine for F16MlPmCoreAfterReset to perform MSR initialization on all
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlPmCoreAfterResetPhase1OnCore\n");
  ...
  )
{
  ...
  IDS_HDT_CONSOLE (CPU_TRACE, "  F16MlPmCoreAfterResetPhase2OnCore\n");

  // 5. Wait for MSRC001_0071[CurCpuFid, CurCpuDid] = [CpuFid, CpuDid] from
  //    MSRC001_00[6B:64] indexed by D18F3xDC[HwPstateMaxVal].
  ...
  // 6. If MSRC001_0071[CurPstateLimit] != MSRC001_0071[CurPstate], wait for
  //    MSRC001_0071[CurCpuVid] = [CpuVid] from MSRC001_00[6B:64] indexed by
  //    MSRC001_0061[HwPstateMaxVal].
  if (((COFVID_STS_MSR *) &LocalMsrRegister)->CurPstateLimit != ((COFVID_STS_MSR *) &LocalMsrRegister)->CurPstate) {
    do {
      LibAmdMsrRead (MSR_COFVID_STS, &LocalMsrRegister, StdHeader);
    } while (GetF16MlCurCpuVid (&LocalMsrRegister) != GetF16MlCpuVid (&TargetPsMsr));
  }
