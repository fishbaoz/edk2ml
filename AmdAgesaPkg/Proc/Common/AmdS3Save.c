  ...
  )
{
  ...
  // Cpu Features
  IDS_PERF_TIMESTAMP (TP_BEGINDISPATCHCPUFEATURESS3SAVE, &AmdS3SaveParams->StdHeader);
  IDS_HDT_CONSOLE (MAIN_FLOW, "DispatchCpuFeatures: S3SaveStart\n");
  AgesaStatus = DispatchCpuFeatures (CPU_FEAT_S3_SAVE_END, &AmdS3SaveParams->PlatformConfig, &AmdS3SaveParams->StdHeader);
  IDS_HDT_CONSOLE (MAIN_FLOW, "DispatchCpuFeatures: S3SaveEnd\n");
  if (AgesaStatus > ReturnStatus) {
    ReturnStatus = AgesaStatus;
  }
  IDS_PERF_TIMESTAMP (TP_ENDDISPATCHCPUFEATURESS3SAVE, &AmdS3SaveParams->StdHeader);
