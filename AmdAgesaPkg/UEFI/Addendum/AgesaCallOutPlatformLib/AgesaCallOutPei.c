  ...
  )
{
  ...
  UINT32                *ApStackBase;
  PSP_SMM_RSM_MEM_INFO  *PspSmmRsmMemInfoPtr;

  PeiServices = (EFI_PEI_SERVICES **)RebaseParams->StdHeader.ImageBasePtr;
  PspSmmResumePpi = NULL;
  ApStackBase = NULL;
...
//  |             Signature (4Bytes)              |
//  |---------------------------------------------|
//  |             mPspSmmRsmMemInfo  (4Bytes)     |
//  +---------------------------------------------+
//  |                                             |
//  |                                             |
//  +---------------------------------------------+
    //Get AP Stack Base
    ApStackBase = (VOID *) ((((UINTN) (VOID *) (&PeiServices))
                                     & (~((RESUME_AP_STACK_SIZE) - 1)))
                                     + RESUME_AP_STACK_SIZE);
    if (ApStackBase == NULL) {
      return AGESA_UNSUPPORTED;
    }
    if (*(--ApStackBase) != 0xABCDABCD) {
      return AGESA_UNSUPPORTED;
    }
    //Get mPspSmmRsmMemInfo->RsmHeapPtr
    PspSmmRsmMemInfoPtr = (PSP_SMM_RSM_MEM_INFO *) (UINTN) (*(--ApStackBase));
    RebaseParams->HeapAddress = (UINTN) (PspSmmRsmMemInfoPtr->RsmHeapPtr);
  } else {
    Status = (*PeiServices)->LocatePpi (PeiServices, &gPspSmmResumePpiGuid, 0, NULL, &PspSmmResumePpi);
    if (EFI_ERROR (Status)) {
      return AGESA_UNSUPPORTED;
    }

    RebaseParams->HeapAddress = (UINTN) PspSmmResumePpi->HeapBase;
  }
  return AGESA_SUCCESS;
}
