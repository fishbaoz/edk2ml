 ...
 *
 * Description: AmdApeiBert.h - Contains structures for APEI (WHEA) related ACPI
...
#define BERT_SIG 0x54524542ul             // "BERT" backwards
#define AMD_BERT 0x5452454220444D41ull    // "AMD BERT" backwards
...
#define REVISION_BERT     0x201

  /// Pack structures on a 1-byte boundary
  #pragma pack(1)

/// APEI Boot Error Region Table
  typedef struct _APEI_BOOT_ERROR_REGION_TABLE {
    UINT32  BlockStatus;                ///< Block Status
    UINT32  RawDataOffset;              ///< Raw Data Offset
    UINT32  RawDataLen;                 ///< Raw Data Length
    UINT32  GenericDataLen;             ///< Generic Data Length
    UINT32  ErrorSeverity;              ///< Error Severity
  } APEI_BOOT_ERROR_REGION_TABLE;

/// BERT Generic Memory Error Entry
  typedef struct _BERT_GENERIC_MEM_ERR_ENTRY {
    GEN_ERR_DATA_ENTRY    GenErrorDataEntry;    ///< Generic Error Data Entry
