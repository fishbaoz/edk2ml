 ...
 * @e project:      AGESA
 * @e sub-project:  PSP
 * @e \$Revision: 285255 $   @e \$Date: 2014-02-18 23:08:50 -0600 (Tue, 18 Feb 2014) $
 *
 */
/*****************************************************************************
 *
...
#include "PspMbox.h"
#include "PspDebug.h"
#include "PspCorruptDetectHob.h"
#include EFI_PPI_DEFINITION (MemoryDiscovered)
#include "Filecode.h"

#define FILECODE UEFI_PSP_DRIVERS_PSPPEI_PSPPEI_FILECODE
  ...
  )
{
  ...
  PSP_DEBUG ("Psp.PspPei.MemoryDiscoveredPpiCallback\n");
  ...
  //Check PSP Recovery required Flag, if set publish GUID Hob
  if (PspMbox->MboxSts.Recovery) {
    PSP_DEBUG ("Recovery Flag Detected\n");
    //Publish GUID HOB
    Status = (*PeiServices)->CreateHob (PeiServices,
                              EFI_HOB_TYPE_GUID_EXTENSION,
                              sizeof (EFI_GUID) + sizeof (BOOLEAN),
                              &GuidHob);
    ASSERT (Status == EFI_SUCCESS);
    EfiCommonLibCopyMem (&GuidHob->Name, &gPspCorruptDetectGuid, sizeof (EFI_GUID));
    GuidHob++;
    //Set Recovery Flag to 1
    *(BOOLEAN *)GuidHob = 1;
  }
  return (EFI_SUCCESS);
}
  ...
  )
{
  ...
  PSP_DEBUG ("Psp.Drv.PspPei Enter\n");
