...
#define FILECODE UEFI_PSP_DRIVERS_PSPSMM_PSPRESUMESERVICES_FILECODE
EFI_LIST_ENTRY   PspHeadList;
  ...
  )
{
  ...
  /// @todo Again this is insecure. Need to install protocol in SMM handle & not BS
  /// IBV need to allow Install/Locate protocol over Smst (not part of PI 0.9
  Handle = NULL;
  ...
  )
{
  ...
  mSmst->SmmFreePool (DispatchHandle);
  return  Status;
}
