  ...
  )
{
  ...
  if (ResumeType == ResumeFromConnectedStandby) {
    Value = 0xDB000001;
    PspLibIoWrite (AccessWidth32, 0xE0, &Value);
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  if (ResumeType == ResumeFromConnectedStandby) {
    Value = 0xDB000002;
    PspLibIoWrite (AccessWidth32, 0xE0, &Value);
    PspLibIoWrite (AccessWidth32, 0xE0, Context);
...
EFI_DRIVER_ENTRY_POINT (NewRsmDrvEntryPoint)
EFI_STATUS
NewRsmDrvEntryPoint (
  ...
  )
{
  ...
  DEBUG ((EFI_D_GENERIC , "NewRsmDrv Enter\n"));
  ...
  DEBUG ((EFI_D_GENERIC , "NewRsmDrv Running in SMM\n"));
