...
#define FILECODE UEFI_FCH_AVALON_DXE_FCHDXEMMIO_FCHDXEMMIO_FILECODE
  ...
  )
{
  ...
  // HPET
  LibFchMemRead ( EfiPciWidthUint32, MmioAddr + PMIO_BASE + FCH_PMIOA_REG00, &ChipBaseAddress);
  if ( ChipBaseAddress & BIT6 ) {
    BaseAddress = 0xFED00000;
    Length = 0x200;
    Status = pMmioProtocol->AmdMmioMapManager (
                                      StdHeader,
                                      BaseAddress,
  ...
  // BIOS RAM
  LibFchMemRead ( EfiPciWidthUint32, MmioAddr + PMIO_BASE + FCH_PMIOA_REG04, &ChipBaseAddress);
  if ( ChipBaseAddress & 0x1 ) {
    BaseAddress = 0xFED10000;
    Length = 0x100;
    Status = pMmioProtocol->AmdMmioMapManager (
                                      StdHeader,
                                      BaseAddress,
