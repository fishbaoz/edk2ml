...
#include "FchDxeLib.h"
#include "Filecode.h"
#define FILECODE UEFI_FCH_AVALON_DXE_FCHDXEAUX_DXEGPIO_FILECODE
  ...
  )
{
  ...
  for (i = 0; i < NumberOfGpio; i++) {
    ...
    GpioMaskValue = 0x8F;
    LibFchMemRw ( EfiPciWidthUint8, MmioAddr + GPIO_BANK0_BASE + (GpioDxeAuxPt[i].Gpio << 2) + 2, &GpioMaskValue, &GpioDxeAuxPt[i].GpioValue);
    GpioMaskValue = 0x7F;
    LibFchMemRw ( EfiPciWidthUint8, MmioAddr + GPIO_BANK0_BASE + (GpioDxeAuxPt[i].Gpio << 2) + 2, &GpioMaskValue, &GpioDxeAuxPt[i].GpioType);
  }
}
