  ...
  )
{
  ...
  while ((UINTN)CurrentPtr < ((UINTN)AmlPtr + (UINTN)PkgLength + 2)) {
    ...
    if ((*Signature == EFI_SIGNATURE_32 ('_', 'C', 'I', 'D')) || (*Signature == EFI_SIGNATURE_32 ('_', 'H', 'I', 'D'))) {
      SegPtr = BlockStart;
      SearchDword = EFI_SIGNATURE_32 ('_', 'S', 'E', 'G');
  ...
  )
{
  ...
  *FieldOp = 0x815B;
  ...
  EfiCopyMem (AmlPtr, (UINT8 *) ((UINTN)OldFieldAml + 2 + (UINTN)ByteCount), OldLength - 2 - (UINT32)ByteCount);
  ...
  EfiCopyMem (AmlPtr, AddFieldAml, Length);
  ...
  )
{
  ...
  *AmlPtr = 0x10;
  ...
  EfiCopyMem (AmlPtr, OldAmlPtr, 0x0D);      //0x0D: copy to Region Offset
