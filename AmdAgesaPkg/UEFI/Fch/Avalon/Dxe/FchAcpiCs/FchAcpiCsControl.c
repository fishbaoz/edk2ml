  ...
  )
{
  ...
  case FCH_ACPI_DEVICE_ASF_SMBUS:
    *PathName = EFI_SIGNATURE_32 ('S', 'M', 'B', '1');
    return EFI_SUCCESS;

  case FCH_ACPI_DEVICE_GPIO:
    *PathName = EFI_SIGNATURE_32 ('G', 'P', 'I', 'O');
    return EFI_SUCCESS;
  ...
  )
{
  ...
  for (CurrentAml = (UINT8 *)Table;
    ...
    if (CheckData (CurrentAml, OldName, NameLength)) {
      EfiCopyMem (CurrentAml, NewName, NameLength);
    }
  }
  return EFI_SUCCESS;
}
  ...
  )
{
  ...
  EfiZeroMem (TempBuffer, NewDataLength);
  EfiCopyMem (TempBuffer, FchAcpiData.FchData, FchAcpiData.DataLength);
  EfiCopyMem ((UINT8 *) ((UINTN)TempBuffer + (UINTN)Offset), Data, Length);
  ...
  EfiZeroMem (TempBuffer, NewAcpiInfoLength);
  EfiCopyMem (TempBuffer, FchAcpiData.AcpiFieldNames, FchAcpiData.AcpiInfoLength);
  ...
  )
{
  ...
  if (AcpiTable != NULL) {
    Signature = EFI_SIGNATURE_32 ('_', 'G', 'P', 'E');
  ...
  for (FchAcpiUpdatePtr = (UINT8 *) ((UINTN)CurrentTable + sizeof (EFI_ACPI_DESCRIPTION_HEADER));
    ...
    case (EFI_SIGNATURE_32 ('F', 'C', 'H', 'D')):
      if (FchIsAmlOpRegionObject (FchAcpiUpdatePtr)) {
        ...
        EfiCopyMem (NewTable, CurrentTable, sizeof (EFI_ACPI_DESCRIPTION_HEADER));
        ...
        EfiCopyMem (FchAcpiUpdatePtr, NewField, NewFieldSize);
        ...
        EfiCopyMem (FchAcpiUpdatePtr, NextBlockPtr, Size);
  ...
  )
{
  ...
  Status = EFI_SUCCESS;
  GpioDevName = EFI_SIGNATURE_32 ('G', 'P', 'I', 'O');
  ResourceName = EFI_SIGNATURE_32 ('R', 'B', 'U', 'F');
  ...
  )
{
  ...
  Status = EFI_SUCCESS;
  AsfDevName = EFI_SIGNATURE_32 ('S', 'M', 'B', '1');
  ResourceName = EFI_SIGNATURE_32 ('_', 'C', 'R', 'S');
  AsfIrq = FchGetAsfIrq ();
