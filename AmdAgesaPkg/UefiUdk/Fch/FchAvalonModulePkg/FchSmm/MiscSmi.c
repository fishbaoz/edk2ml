  ...
  )
{
  ...
  FchEhciDebugPortService (pFchPolicy);
  return Status;
}
...
{
  EFI_STATUS                               Status;
  FCH_SMM_USB_DISPATCH2_PROTOCOL           *FchSmmUsbDispatchProtocol;
  EFI_HANDLE                               UsbHandle;

  UsbHandle = NULL;

  //
  //  Locate SMM dispatch protocol
  //
  Status = gSmst->SmmLocateProtocol (
  ...
  if (!EFI_ERROR (Status)) {
    ...
    FchSmmUsbRegisterContext.Order = 0x80; //first
    FchSmmUsbRegisterContext.Device = 0;
    FchSmmUsbRegisterContext.Type = 0;

    Status = FchSmmUsbDispatchProtocol->Register (
                                          FchSmmUsbDispatchProtocol,
                                          (FCH_SMM_USB_HANDLER_ENTRY_POINT2)AmdEhciDebugPortCallback,
